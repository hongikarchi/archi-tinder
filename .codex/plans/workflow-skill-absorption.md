...
- `.codex/agents/git-manager.toml` → add `deprecated: true` to frontmatter + body note: `# DEPRECATED — superseded by .agents/skills/git-commit/. Kept for fallback during the 2026-05-26 migration. Slated for removal in a follow-up PR after 1 week of skill-only usage.`
- `.codex/agents/reporter.toml` → same pattern, pointing to `.agents/skills/reporter-inline/`.
- `.codex/agents/git-publisher.toml` → no marker (kept indefinitely for edge cases).
...
Before writing `reporter-inline/SKILL.md`, enumerate every behavior from `.codex/agents/reporter.toml` that must survive:
...
| Step 4f | `agents[]` from `.codex/agents/*.toml` frontmatter | ✓ — and now includes deprecated marker if present |
...
Step 0 — Publish gate (HARD precondition)
  ...
  (b) An active `.codex/plans/<slug>.md` file in scope authorizes the action.
...
### 1. `.agents/skills/reporter-inline/SKILL.md`
...
### 2. `.agents/skills/git-commit/SKILL.md`
...
### 3. `.agents/skills/git-publish/SKILL.md`
...
- Skill guards mirror the existing HARD RULE 1-4 (branch protection, force/no-verify/history-rewrite prohibition) — see each `.agents/skills/<skill>/SKILL.md`.
```
