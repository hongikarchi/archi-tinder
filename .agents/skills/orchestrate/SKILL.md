---
name: orchestrate
description: Use this skill to implement any feature or fix end-to-end. It reads the project context, breaks the task into backend/frontend specs, dispatches back-maker and front-maker sub-agents, runs code-review and security-manager in parallel, manages the fix loop (max 2 cycles), then runs the git-commit skill (commit), runs browser-verify directly with the in-app browser (runtime + drift gate), runs the reporter-inline skill (audit), and runs the git-publish skill (push/PR/merge). The git-publisher agent is reserved for Mode 3 deploy, external PR triage, and complex rebase recovery.
---
...
Every "Dispatch `<agent>`" instruction in this file means: **call the Codex
sub-agent tool `multi_agent_v1.spawn_agent` with `agent_type: "<agent>"`**.
...
- `Task` — obsolete name from older Codex; no longer exists. Use `spawn_agent`.
...
```json
{
  "agent_type": "back-maker",
  "message": "<full self-contained brief for the subagent>"
}
```
...
For parallel dispatches (Step 4: code-review + security-manager), spawn both
agents before waiting so they run concurrently.

If `spawn_agent` returns an error, report the error to the user and stop — do NOT
bypass delegation by writing source code yourself. The no-direct-code rule in
§Rules below is absolute.

## Before every task
1. Read `AGENTS.md` — conventions, rules, DB schema, coding standards, and `## Product Identity` + `## Product Constitution` (the vision + acceptance + decision principles anchor)
2. Read `.codex/Task.md` — current problem board (`## Now` / `## Next` / `## Done`); Phase 16-18 dimensions live in `## Next` directly (the prior `docs/specs/*.md` folder was absorbed 2026-05-24)
3. Read code directly — the running code is the source of truth for architecture and API surface (per AGENTS.md `## What This Repo Does`). No standalone Report.md.
4. If algorithm task: read `docs/algorithm.md` for theory + production hyperparameters
5. If task references a Phase or open question: read the matching `#### <SLUG>` entry under one of the `### HIGH` / `### MEDIUM` / `### LOW` buckets in `.codex/Task.md` `## Next`

## When user requests work
1. **Session start (Now/Next discipline)** — open `.codex/Task.md`. Read `## Now` first.
   ...
   - If the user's request is brand-new: write a fresh `### <ID> — <Korean title>` directly into `## Now` using the ID convention from `.codex/Task.md ## Workflow Rules` (e.g. `BACK-LLM-1`, `FRONT-UX-1`, `INFRA-DB-1`).
2. Read `AGENTS.md` `## Product Identity` + `## Product Constitution` + scan relevant code.
...
Follow the `## Now` / `## Next` discipline at the top of `.codex/Task.md`:
1. Read `## Now` first. If non-empty, continue that entry.
2. If empty, pull the highest-priority item from `## Next ### HIGH` and promote it to `## Now` (cut from Next, paste into Now, raise heading level one — see `.codex/Task.md ## Workflow Rules`).
...
### Step 5 — Decision
**If both PASS:**
→ Check architectural fit yourself: does this match `AGENTS.md` `## Product Identity`
...
browser verification failures, and architectural-fit rejections all draw from the same budget
...
### Step 7 — Runtime browser verification + drift gate
Run the `browser-verify` skill directly in the main session. Use the Codex in-app
browser; do not dispatch a browser-test agent by default.

Pick mode by risk:
- SKIP: pure docs/policy/config with no runtime surface.
- SMOKE: app boots and target page loads.
- FEATURE-SCOPED: changed-surface checklist.
- FULL-SWIPE: recommendation/swipe path changes.

The verification returns PASS / FAIL / SKIPPED / NOT RUN / ABORTED.

- **PASS or SKIPPED:** → Step 8
- **FAIL:** treat as a code-review FAIL — go to the Fix Loop (Step 5b). Browser
  verification failures count toward the shared 2-cycle limit. The fix cycle
  re-runs back-maker / front-maker → code-review + security-manager →
  `git-commit` skill (a new commit on the same feature branch) → browser-verify.
- **ABORTED drift (`origin/develop` moved):** no code fix is needed. Inform the
  user, sync/rebase as appropriate, then re-run browser-verify. Drift does NOT
  count toward the 2-cycle limit.
- **NOT RUN (local dev server not running):** note it in the report and proceed
  only if the change is not runtime-sensitive; otherwise start servers and re-run.
...
- **(b) Active plan with `## PR Plan` section** — if a plan file `.codex/plans/<name>.md` is active for this work and contains an explicit `## PR Plan` section listing N slices, the plan acts as authorization for those N PRs. Each slice's commit may proceed to publish automatically. After the last planned slice, the gate closes (returns to default).
- **(c) In-flight fix-loop** — if `browser-verify` or `code-review` already gated this work in the current dispatch and a tiny follow-up commit is the result of the fix-loop, that continues the original (a) or (b) authorization. No fresh trigger needed.
...
Once the gate opens, proceed to Step 9 (audit-then-publish). The default publish path is the `git-publish` skill. Escalate to the `git-publisher` agent only for the edge cases listed in AGENTS.md `## Git Operations — HARD RULE`: Mode 3 develop→main deploy, external collaborator PR triage, complex rebase conflicts, push rejection with unclear cause, mid-merge failure.
...
2026-05-26.** The skill updates `.codex/Task.md`, `project/state.js`, and
...
1. Move completed tasks from `.codex/Task.md` `## Now` / `## Next` into
...
what was implemented, the commit/PR, the browser-verify verdict, and any open
follow-ups.

## Algorithm work — externally owned

Per `AGENTS.md` `## Rules` (`docs/algorithm.md` narrow write permission, codified
...
- If you notice a `AGENTS.md` convention that needs updating, propose the change in
...
- **Direct work is acceptable** for meta/infra/tooling (`tools/*.sh`, `hooks/*`,
  ...
  sub-MINOR follow-ups, docs/policy edits to `AGENTS.md` / `CONTRIBUTING.md` /
  `.codex/agents/*.toml` / `.agents/skills/*` / `docs/*`), one-line trivial fixes, and
...
- **Token-saving rules** — see `.codex/WORKFLOW.md` § Token-saving rules:
