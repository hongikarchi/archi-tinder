...
export function GalleryOverlay({ card, onClose, fullscreen = false }) {
  ...
  if (fullscreen) {
    return (
      ...
      >
        ...
        {/* 이미지 개수 표시 */}
        {gallery.length > 0 && (
          <p style={{ color: '#4b5563', fontSize: 11, textAlign: 'center', padding: '8px 0 12px', margin: 0, flexShrink: 0 }}>
            {gallery.length}장 · 위아래로 스크롤
          </p>
        )}
      </div>
    )
  ...
  return (
    ...
    >
      ...
      {/* 이미지 개수 표시 */}
      {gallery.length > 0 && (
        <p style={{ color: '#4b5563', fontSize: 11, textAlign: 'center', padding: '6px 0 10px', margin: 0 }}>
          {gallery.length}장 · 옆으로 스크롤
        </p>
      )}
    </div>
  )
...
function Tag({ label, value }) {
  return (
    <span style={{
      ...
      border: '1px solid rgba(255,255,255,0.28)', color: '#fff', whiteSpace: 'nowrap',
    }}>
      <span style={{ color: 'rgba(255,255,255,0.5)', fontSize: 9, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
        {label}
      </span>
      {value}
    </span>
...
function LoadingCard() {
  return (
    <div style={{
      ...
      <div style={{ color: '#4b5563', fontSize: 13 }}>불러오는 중...</div>
    </div>
  )
}
...
export default function SwipePage({ currentCard, progress, isCompleted, isLoading, projectName, onSwipe, onViewResults }) {
  ...
  if (isCompleted) {
    return (
      <div style={{
        ...
        <h2 style={{ color: '#fff', fontSize: 22, fontWeight: 800, margin: 0 }}>All done!</h2>
        <p style={{ color: '#6b7280', fontSize: 14, textAlign: 'center', margin: 0 }}>
          {`"${projectName || 'Project'}" 스와이프 완료`}
        </p>
    ...
    return (
      <div style={{
        ...
        <p style={{ color: '#6b7280', fontSize: 14, textAlign: 'center' }}>
          조건에 맞는 건물이 없습니다.<br />필터를 변경해 보세요.
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col items-center justify-between min-h-screen bg-[#0f0f0f] px-4 py-8">
      ...
      <div style={{ textAlign: 'center', width: '100%' }}>
        ...
        <div style={{ maxWidth: CARD_WIDTH, margin: '0 auto' }}>
          ...
          <div style={{ height: 4, borderRadius: 999, background: '#1f2937', overflow: 'hidden' }}>
            <div style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
        <p style={{ color: '#374151', fontSize: 11, margin: 0 }}>← skip · tap to flip · save →</p>
        <div style={{ display: 'flex', gap: 32 }}>
          <button
            ...
            style={{
              ...
              color: isLoading ? '#374151' : '#fff', fontSize: 22, cursor: isLoading ? 'default' : 'pointer',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
            }}
            aria-label="Dislike"
          >✕</button>
