# Make Web Service -- Claude Code Instructions
  ...
  **Hard rules — NEVER violate:**
  ...
  4. **Never use `--no-verify`, `--force`, `--force-with-lease`, `git rebase -i`, `git reset --hard` on shared branches**, or any history-rewriting flag. **Single carve-out — post-deploy develop force-reset (Bug #5)**: immediately after a successful `develop → main` squash deploy merge, `origin/develop` MUST be force-reset to match `origin/main` to prevent commit-graph divergence that breaks the next deploy PR (exact command + safety checks: `.claude/agents/git-publisher.md` § Mode 3). This is the ONLY permitted force on a shared branch and applies ONLY in the immediate post-deploy window. Precondition: every commit on `origin/develop` must be content-equal to `origin/main` (no unmerged in-flight feature PR targets `develop`).
  ...
  7. **One clone per worker — never operate in another worker's directory (concurrent-agent isolation).** Every worker (remote human OR local AI tool) owns ONE working directory with its OWN `.git`, on its own `feature/*` branch, with its own PR. **Claude = the main clone `make_web/`** (tendency backend/API/algorithm/DB — a default, not a hard wall); **Codex = its own separate clone `make_web-codex/`** (browser, UI/UX), never the main clone. **No `git worktree` for session isolation** — worktrees share one `.git` (2026-05-31: a Codex worktree session moved the main checkout's `HEAD`); separate clones are structurally immune. The Agent tool's `isolation:"worktree"` (parallel sub-agents within one session) is unrelated. **Session-start check**: confirm you are in `make_web/` on `develop` or a `feature/claude-*` branch; wrong directory → switch, never `git reset`. Full model + incident detail: `CONTRIBUTING.md` § "Concurrent agents — one clone per worker".
  ...
  **If `git status` at session start shows you are on `main` or `develop` with uncommitted changes**: the previous session likely did not switch to a feature branch. Stash or save the work, then create a proper feature branch before continuing. Do not stage or commit while on a protected branch.
  ...
  ## Workflow — ultracode-main: one session + feature workflow + agents + skills

  ArchiTinder Make Web's Claude side runs as **one Claude Code orchestrator session on the top session-model tier** (Fable 5 as of 2026-08; concurrent with Codex in its own clone — HARD RULE 7). It owns architecture, schema, auth, product + release decisions, and review — it does not write feature code itself. The primary workflow is **ultracode = the Workflow tool**: the build+review CORE runs as a deterministic multi-agent Workflow script, and the session gates around it. Trivial mechanical edits stay solo (no workflow).
  ...
  - **Model tier map** (cost control = tiering inside the workflow, not avoiding fan-out): **session model (Fable 5)** = orchestrator; **Opus** = adversarial-verify/judge — always pinned with `effort: 'high'` (an xhigh session inherits its effort into `agent()` and Opus without a thinking budget 400s); **Sonnet 4.6** = back/front-maker, code-review, security-manager, app-test; **Haiku 4.5** = exploration/search. Every workflow `agent()` call MUST pin `model` explicitly — `agentType` does not carry the frontmatter tier; the default is inherit-session-model, which silently runs every worker on the most expensive tier.
  ...
  - Every building query MUST gate on `is_publishable = true` (~6-7% of rows are non-publishable; census + semantics in `docs/database-schema.md`). `engine._build_filter_sql` already emits this clause; raw SQL elsewhere must add it.
  ...
  **Working principle**: problem definition first → plan as a written artifact → architecture before tactical code → foundational correctness (data shape, matching algorithm, feedback loop) over polish.
  ...
  - Run: `cd backend && python manage.py runserver 8001`. (`python`, not `python3` — the Windows `python3` is a broken Store stub, same class as Makefile #293.)
