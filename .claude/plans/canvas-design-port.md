...
8. **Logout stays in the top-right cluster on EVERY page** (user, 2026-09-06 — deliberate deviation from the mocks). The mock system shows logout only in the profile page's top-left cluster; the user chose accessibility over fidelity. **Standing exception**: vision/property diffs will keep flagging an "extra logout circle after the theme pill" on ~15 boards — classify as intentional, never "fix" it. The profile page keeps its left-cluster logout as the mock shows AND omits the top-right one (PageTopControls without onLogout there) — that page is already converged.
9. **user-other shows Liked stat + Liked/Created tabs to visitors** (user, 2026-09-06 — mock-faithful). Liked lists become readable by any authenticated user. Backend exposure added on feature/claude-design-finetune.
10. **Upload gains the mock's Built/Unbuilt status toggle + collapsible 선택 정보 card** (user, 2026-09-06). Work.status field + migration — `make migrate-local` required before local testing.
