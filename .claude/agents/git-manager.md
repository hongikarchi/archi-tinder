...
3. Stage all modified and new files — excluding secret/cache files:
   ```bash
   git add --all -- \
     ':(exclude).env' \
     ':(exclude).env.*' \
...
- All standard tracked files (`backend/`, `frontend/`, `docs/`, `DESIGN.md`, `CLAUDE.md`, `CONTRIBUTING.md`, `.claude/`, `tools/`, `hooks/`, `.github/`, etc.) are stageable. The exclude list above is just for secrets and build artifacts.
