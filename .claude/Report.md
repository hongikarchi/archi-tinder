...
## Last Updated (Claude)
- **Date:** 2026-04-25
- **Commits:** f17cb5e -- feat: pool exhaustion guard with auto 3-tier relaxation (Sprint 0 A4)
- **Phase:** Sprint 0 A4 -- Pool exhaustion guard (§5.6 + §6 implementation requirement)
- **Changes:**
  - `backend/apps/recommendation/engine.py` -- create_pool_with_relaxation() helper factors out 3-tier logic (full → drop geo/numeric → random) for reuse; refresh_pool_if_low() runs from SwipeView when remaining pool < 5, escalates to next tier and merges new candidates with exclude_ids = pool_ids + exposed_ids.
  - `backend/apps/recommendation/migrations/0008_analysissession_pool_relaxation_state.py` -- new migration adding 4 fields to AnalysisSession (pure AddField, no data migration).
  - `backend/apps/recommendation/models.py` -- AnalysisSession gains 4 new fields: original_filters, original_filter_priority, original_seed_ids, current_pool_tier.
  - `backend/apps/recommendation/views.py` -- SessionCreateView inline 3-tier replaced with engine.create_pool_with_relaxation() call (identical behavior); SwipeView calls refresh_pool_if_low() in normal swipe path AND action-card "Reset and keep going" branch; update_fields extended with pool_ids, pool_scores, current_pool_tier.
  - `backend/tests/test_sessions.py` -- New TestPoolExhaustionGuard class (5 tests). 49 total tests pass.
- **Verification:** 49 backend tests pass (44 prior + 5 new). Reviewer: PASS. Security: PASS.
- **Change diagram:**
```mermaid
graph TD
    subgraph Backend
        ...
        0008_migration.py:::new
    end
    engine.py --> views.py
    models.py --> 0008_migration.py
