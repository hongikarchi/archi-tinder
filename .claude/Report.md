...
## Last Updated (Claude)
- **Date:** 2026-04-25
- **Commits:** de9bfa3 -- feat: Topic 04 DPP greedy MAP + MMR lambda ramp (Sprint 4)
- **Phase:** Sprint 4 Topic 04 -- DPP greedy MAP + MMR lambda ramp
- **Changes:**
  - `backend/apps/recommendation/engine.py` -- compute_mmr_next: λ ramp logic (λ(t) = λ_base · min(1, |exposed|/N_ref)) when mmr_lambda_ramp_enabled; new compute_dpp_topk(candidates, embeddings, q_values, k, alpha) Wilhelm 2018 kernel + Chen 2018 Cholesky-incremental greedy MAP O(N·k²); α clamped [0,1]; singularity pad + 2-phase fallback; embeddings via get_pool_embeddings (not card dicts).
  - `backend/apps/recommendation/views.py` -- SessionResultView DPP block after Topic 02 rerank (cosine→rerank→DPP order); gated on dpp_topk_enabled + len>=2 + session.like_vectors; import numpy hoisted to module top.
  - `backend/config/settings.py` -- RECOMMENDATION dict gains 5 entries (mmr_lambda_ramp_enabled F, mmr_lambda_ramp_n_ref 10, dpp_topk_enabled F, dpp_alpha 1.0, dpp_singularity_eps 1e-9).
  - `backend/tests/test_topic04.py` -- NEW: 15 tests (4 ramp + 7 DPP + 3 integration + 1 early-return). 119 total pass + 1 skipped.
- **Verification:** 119 backend tests pass + 1 skipped (104 prior + 15 new). Reviewer: PASS. Security: PASS.
- **Change diagram:**
```mermaid
graph TD
    subgraph Backend
    ...
    engine.py --> test_topic04.py
    views.py --> test_topic04.py
    settings.py --> engine.py
