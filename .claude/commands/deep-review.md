---
description: Deep commit-level review of current branch vs main — architecture, optimization, security, test coverage, cross-commit drift. Writes report to .claude/reviews/. On PASS, performs HEAD and origin/main drift checks and emits REVIEW-PASSED (ready for manual `git push` from this terminal), REVIEW-ABORTED (drift), or REVIEW-FAIL to Task.md Handoffs.
...
current branch, persist it to `.claude/reviews/`, run the HEAD/`origin/main`
drift checks described in Step 6, and emit a one-line handoff signal
(`REVIEW-PASSED`, `REVIEW-ABORTED`, or `REVIEW-FAIL`) to the
`## Handoffs` section of `.claude/Task.md`. Do not modify source code.
Do not commit. Do not push — on `REVIEW-PASSED`, the user runs `git push`
manually from this same review terminal (no context-switch back to main).
