...
54. **S2** -- DB new-schema integration -- COMPLETED 2026-05-14 *(Make DB owner pushed `canonical_v2_buildings` 31-col / 39,776-row table; Make Web cutover via `feature/admin-s2-new-schema`: engine.py 20+ raw SQL rewritten + canonical_bld_id PK + is_publishable gate + image_focus jsonb cover model; SwipeEvent migration 0018; frontend normalizeCard / fallback chain; full pytest 599/599 GREEN; front-validate GREEN)*
