...
- [SPEC-READY 2026-04-26] requirements-spec-v1.4 — consolidated search flow requirements + Section 11 actionable directives per topic + **Section 11.1 implementation issues IMP-1..IMP-4** (correctness bug + plumbing gaps + corpus label gate + Gemini thinking-mode push-gate fix). v1.4: §4 time-to-first-card budget ratified to <5s reflecting Sprint 1 prompt empirical floor. 15 post-spec investigations absorbed; investigations 16 (Gemini context caching) + 17 (2-stage decouple) in flight. **Entry**: `research/spec/requirements.md` (binding). Reference: `research/investigations/README.md` (post-spec investigations index — implementation guidance per topic), `research/spec/research-priority-rebaselined.md` (roadmap recommendation, non-binding), `research/search/**` (reasoning archive). Main terminal: read spec, scan latest SPEC-UPDATED in Handoffs for incremental changes, plan independently, implement via orchestrator pipeline.
