...
### Phase 15 SOC3: Office follow (backend + frontend) -- 2026-05-06

#### SOC3-back. OfficeFollow model + OfficeFollowView + is_following injection (e397317) (2026-05-06)
New `OfficeFollow` model with FK to UserProfile + Office, signal-driven `Office.follower_count` counter cache (Greatest underflow floor), `OfficeFollowView` POST/DELETE (IsAuthenticated + FollowWriteThrottle 60/min shared-scope), get_or_create idempotent + filter().delete() race-safe, `OfficeDetailView` GET extended with `is_following` injection, migration 0004_officefollow AddModel-only reverse-safe. 10 new tests (POST creates+increments / idempotent / 404 / DELETE removes+decrements / unauth 401 / is_following toggle / throttle 429). 567 → 577 passed + 1 skipped.
- [x] `apps/profiles/models.py`: OfficeFollow (FK UserProfile + Office, unique_together, signal handlers)
- [x] `apps/profiles/views.py`: OfficeFollowView (POST/DELETE, IsAuthenticated), OfficeDetailView extended with is_following injection
- [x] `apps/profiles/migrations/0004_officefollow.py`: AddModel-only, reverse-safe
- [x] `apps/profiles/tests/test_office_follow.py`: 10 tests across throttle / follow / unfollow / is_following / 404 / 401
- Commit: e397317

#### SOC3-front. FirmProfilePage follow button wiring (39de1d4) (2026-05-06)
Replaces TODO-stub `handleToggleFollow` with optimistic update + rollback + `isFollowingPending` race guard + server-authoritative `follower_count` override; new `followOffice`/`unfollowOffice` API wrappers mirror `followUser`/`unfollowUser`. Defense-in-depth via existing officeId regex.
- [x] `frontend/src/pages/FirmProfilePage.jsx`: handleToggleFollow wired, optimistic state, rollback on error
- [x] `frontend/src/api/social.js`: followOffice + unfollowOffice wrappers
- [x] lint + build clean; REVIEW-PASSED: 39de1d4
- Commit: 39de1d4

### Phase 14 BOARD3: BoardDetailPage frontend integration -- 2026-05-06

#### BOARD3-front. BoardDetailPage wired to GET /projects/{uuid}/ + reaction toggle (aedc817) (2026-05-06)
New `useBoard` hook with cancellation guard; `getProject` / `getBoardBuildings` / `reactToProject` / `unreactToProject` API wrappers; UUID regex defense-in-depth; optimistic reaction toggle + rollback + `isReactionPending` race guard + dedicated `reactionError` banner; server-authoritative `{reaction_count, reacted}` overrides on POST response; MOCK_BOARD preserved verbatim as designer reference. First real-world dispatch through stateful 4-workspace cmux infra.
- [x] `frontend/src/pages/BoardDetailPage.jsx`: wired to real API, reaction toggle, error banner
- [x] `frontend/src/api/projects.js`: getProject, getBoardBuildings, reactToProject, unreactToProject
- [x] `frontend/src/hooks/useBoard.js`: NEW, cancellation guard
- [x] lint + build clean; REVIEW-PASSED: 5fbf1fa
- Commit: aedc817

### Phase 14 BOARD2: Visibility selection in project creation -- 2026-05-06

#### BOARD2. Public/private toggle in ProjectSetupPage (bdc8d7b) (2026-05-06)
Adds public/private toggle to ProjectSetupPage with default 'private'; visibility flows through wizardData → LLMSearchPage → `handleStart` → fire-and-forget `PATCH /projects/{id}/ {visibility}` after session-create when `visibility !== 'private'`. Reviewer cycle 1 fix: `handleLogin` project mapping preserves `p.visibility || 'private'` (was dropped, causing lock icon misread).
- [x] `frontend/src/pages/ProjectSetupPage.jsx`: visibility toggle UI
- [x] `frontend/src/pages/LLMSearchPage.jsx`: visibility prop wired to handleStart
- [x] `frontend/src/api/projects.js`: updateProject wrapper
- [x] `frontend/src/App.jsx`: handleLogin project mapping fix
- [x] lint + build clean; REVIEW-PASSED: bdc8d7b
- Commit: bdc8d7b

### Cmux 4-workspace stateful infrastructure -- 2026-05-06

#### CMUX. Stateful 4-workspace cmux setup replacing stateless codex exec (a379bc6) (2026-05-06)
Pivots from stateless `codex exec` pattern to persistent stateful Codex CLI sessions in 4 tmux workspaces (WEB-MAIN / WEB-BACK / WEB-FRONT / WEB-REVIEW). `tools/cmux_setup.sh` idempotent creator + init prompts; `dispatch.sh` task sender with >1500-char file-fallback; `poll.sh` screen reader; `AGENTS.md` baseline; `team-back.md` + `team-front.md` per-team owned-files + DRF gotchas + 7-axis/6-axis self-review checklists.
- [x] `tools/cmux_setup.sh`: idempotent 4-workspace creator
- [x] `tools/dispatch.sh`: task sender; >1500-char messages auto-written to /tmp/ file-pointer
- [x] `tools/poll.sh`: screen reader for WEB-MAIN polling
- [x] `AGENTS.md`: Codex baseline + hard guardrails
- [x] `.claude/agents/team-back.md`: backend DRF gotchas + 7-axis self-review checklist
- [x] `.claude/agents/team-front.md`: frontend conventions + 6-axis self-review checklist
- Commits: a379bc6 (cmux infra) + 3ef52b2 (comment cleanup) + 51dd387 (stateless protocol, superseded)

### Hybrid pre-commit policy -- 2026-05-06

#### HYBRID-REVIEW. Codex team self-review replaces in-session reviewer+security on dispatched work (756b247) (2026-05-06)
When work dispatched to WEB-BACK/WEB-FRONT, WEB-MAIN trusts Codex team's own self-review (7-axis backend / 6-axis frontend checklists in team-back.md / team-front.md) and skips in-session Claude reviewer + security-manager. Cross-model verification still happens at /review (Claude Opus vs Codex). Risky-commit override: teams append `(claude-review-requested)` to trigger in-session pass. Estimated ~150-200K token savings per BOARD-class deliverable.
- [x] `.claude/agents/team-back.md`: 7-axis self-review checklist + risky-zone thresholds
- [x] `.claude/agents/team-front.md`: 6-axis self-review checklist
- [x] `AGENTS.md`: hybrid policy codified
- [x] `CLAUDE.md`: cross-reference to hybrid policy
- [x] REVIEW-PASSED: 756b247
- Commit: 756b247

### dispatch.sh long-message file-fallback -- 2026-05-06

#### DISPATCH-FALLBACK. >1500-char dispatch messages auto-written to /tmp/ file-pointer (5fbf1fa) (2026-05-06)
Empirical fix for cmux send silent-truncation discovered during BOARD3 rollout (2.3 KB plan dropped). Messages >1500 chars written to `/tmp/dispatch-<team>-<ts>.md`; dispatch.sh sends file-pointer instead of inline text.
- [x] `tools/dispatch.sh`: file-fallback for >1500-char messages
- [x] REVIEW-PASSED: 5fbf1fa (bundled with BOARD3 commit)
- Commit: 5fbf1fa

### display_name + bio validation fix -- 2026-05-06

#### VALIDATION-FIX. DRF trim_whitespace=False + custom validators on /users/me/ PATCH (27fee9b) (2026-05-06)
`display_name` + `bio` fields declared with `trim_whitespace=False, allow_blank=True` to prevent DRF default-strip masking custom validators. Fix surfaced by BRANCHING.md DRF gotcha entry codification.
- [x] `backend/apps/accounts/serializers.py`: display_name + bio CharField trim_whitespace=False
- [x] `backend/apps/accounts/tests/`: whitespace-only tests added
- [x] REVIEW-PASSED: 3ef52b2 (bundled in 9-commit range)
- Commit: 27fee9b

### SOC2 reactors list endpoint -- 2026-05-06

#### SOC2-LIST. GET /projects/{id}/reactors/ visibility-gated list endpoint (042bed4) (2026-05-06)
`GET /projects/{id}/reactors/` (public: open; private+owner: open; private+non-owner: 403) mirrors POST visibility gate logic. New `getProjectReactors` API wrapper + `useProjectReactors` paginated hook with cancellation guard.
- [x] `backend/apps/recommendation/views/projects.py`: ReactorsListView, visibility gate mirrors POST
- [x] `frontend/src/api/projects.js`: getProjectReactors wrapper
- [x] `frontend/src/hooks/useProjectReactors.js`: NEW, paginated, cancellation guard
- [x] REVIEW-PASSED: 3ef52b2 (bundled in 9-commit range)
- Commit: 042bed4

### Profile component refactor -- 2026-05-06

#### REFACTOR-PROFILE. Extract 6 shared components from UserProfilePage + FirmProfilePage (b272f37) (2026-05-06)
UserProfilePage 1023 → 565 LOC, FirmProfilePage 962 → 595 LOC. New `frontend/src/components/profile/` package: InfoCol / BoardCard / BioPersonaFlipCard / DescriptionAboutFlipCard / ProjectCard / ArticleCard. Bonus: officeId `/^[A-Za-z0-9_-]{1,64}$/` defense-in-depth guard. Behavior unchanged, inline styles preserved verbatim.
- [x] `frontend/src/components/profile/`: 6 new shared components
- [x] `frontend/src/pages/UserProfilePage.jsx`: 1023 → 565 LOC
- [x] `frontend/src/pages/FirmProfilePage.jsx`: 962 → 595 LOC
- [x] ESLint + Vite build clean; REVIEW-PASSED: 3ef52b2
- Commit: b272f37

### views.py → views/ package split -- 2026-05-02

#### REFACTOR-VIEWS. recommendation/views.py 1700 LOC → 8 modules in views/ package (1ae4684) (2026-05-02)
Zero behavior change. 8 modules: `__init__`, `_shared`, `projects`, `sessions`, `swipe`, `search`, `reports`, `telemetry`. Reduces merge-conflict surface for 3-developer collaboration. 546 tests pass unchanged.
- [x] `backend/apps/recommendation/views/`: 8-module package
- [x] `backend/apps/recommendation/views.py`: replaced by package (barrel re-export for backward compat)
- [x] Existing import statements preserved via re-export barrel
- Commit: 1ae4684

### client.js → api/ package split -- 2026-05-02

#### REFACTOR-CLIENT. client.js 442 LOC → 8 modules under api/ (9c04887) (2026-05-02)
Zero behavior change. 8 modules: `client` (barrel), `core`, `auth`, `images`, `sessions`, `projects`, `profiles`, `social`. ESLint clean, Vite build clean.
- [x] `frontend/src/api/`: 8-module package
- [x] `frontend/src/api/client.js`: barrel re-export for backward compat
- Commit: 9c04887
