...
def create_session(request, profile, recent_cutoff):
    ...
    # TASTE-CALIBRATION-1: confidence-based branching.
    # Frontend posts confidence_score from the preceding parse-query call.
    # <0.60 → create session in chat_initializing phase (no pool build yet).
    # >=0.60 → fall through to the existing exploring path (pool + cards).
    _raw_confidence = request.data.get('confidence_score')
    _confidence_score = None
    if isinstance(_raw_confidence, (int, float)) and 0.0 <= float(_raw_confidence) <= 1.0:
        _confidence_score = float(_raw_confidence)

    if _confidence_score is not None and _confidence_score < 0.60:
        # Extract calibration metadata from request (parse-query output passed through by FE)
        _VALID_SYSTEM_ACTIONS = {'REQUEST_PRIORITY', 'CONFIRM_SELECTION', 'NONE'}
        _raw_system_action = request.data.get('system_action', 'REQUEST_PRIORITY')
        _system_action = (
            _raw_system_action
            if _raw_system_action in _VALID_SYSTEM_ACTIONS
            else 'REQUEST_PRIORITY'
        )
        _llm_response_message = (request.data.get('llm_response_message') or '').strip()
        _suggested_quick_replies = request.data.get('suggested_quick_replies') or []
        if not isinstance(_suggested_quick_replies, list):
            _suggested_quick_replies = []
        _priority_ordered = request.data.get('priority_ordered') or []
        if not isinstance(_priority_ordered, list):
            _priority_ordered = []
        _extracted_metadata = {
            k: filters.get(k)
            for k in ('program', 'material', 'style', 'atmosphere', 'location_country',
                      'color_tone', 'typology_primary')
        }
        _extracted_metadata['priority_ordered'] = list(_priority_ordered)
        ...
        with transaction.atomic():
            ...
            session = AnalysisSession.objects.create(
                ...
                current_pool_tier=1,
                v_initial=None,
                original_q_text=q_text_param,
                confidence_score=_confidence_score,
                extracted_metadata=_extracted_metadata,
                priority_ordered=list(_priority_ordered),
            )

        # Finding #2 (TASTE-CALIBRATION-1 fix): seed conversation_history with the
        # initial user query so that the first calibrate() call has full context.
        # Without this, parse_query on the first calibrate call only sees the
        # calibrate message, losing the original query that drove the low-confidence
        # chat_initializing branch.
        _seed_turns = []
        if raw_query:
            _seed_turns.append({'role': 'user', 'text': raw_query})
        if _llm_response_message:
            _seed_turns.append({'role': 'model', 'text': _llm_response_message})
        if _seed_turns:
            project.conversation_history = {'turns': _seed_turns}
            project.save(update_fields=['conversation_history', 'updated_at'])
        ...
        return Response({
            ...
            'extracted_metadata': _extracted_metadata,
            'llm_response_message': _llm_response_message,
            'suggested_quick_replies': _suggested_quick_replies,
            'cards': [],
            'next_image': None,
            'prefetch_image': None,
            'prefetch_image_2': None,
    ...
    return Response({
        ...
        'confidence_score': _confidence_score if _confidence_score is not None else 1.0,
        'system_action':   'NONE',
        'extracted_metadata': {},
        'llm_response_message': '',
        'suggested_quick_replies': [],
        'cards':           [first_card] if first_card else [],
        'next_image':      first_card,
...
# ---------------------------------------------------------------------------
# CalibrateView orchestration (TASTE-CALIBRATION-1)
# ---------------------------------------------------------------------------

_CALIBRATION_TURN_CAP = 3  # After this many turns, force-promote to exploring


def calibrate_session(request, session):
    """POST /api/v1/analysis/sessions/<id>/calibrate/

    Accepts a user message (free text or quick-reply chip) and appends it to the
    session's accumulated conversation.  Calls parse_query on the full accumulated
    conversation, re-scores confidence, updates extracted_metadata/priority_ordered,
    and decides whether to:
      a) Stay in chat_initializing (confidence still <0.60 and turn cap not reached)
         → returns next clarification question + quick_replies, no cards.
      b) Transition to exploring (confidence >=0.60 OR turn cap reached)
         → builds pool, sets phase='exploring', returns first card batch.

    Body: {message: str}
    """
    # Validate session phase
    if session.phase not in ('chat_initializing', 'exploring'):
        return Response(
            {'detail': 'Session is not in a calibratable phase.'},
            status=status.HTTP_400_BAD_REQUEST,
        )
    if session.phase == 'exploring':
        # Already exploring — return current state, no re-calibration needed.
        return get_session_state(request, session)
    ...
    # Build accumulated conversation history from project.conversation_history
    project = session.project
    conv_history = project.conversation_history
    if not isinstance(conv_history, dict):
        conv_history = {}

    # Retrieve stored turns (list of {role, text})
    turns = conv_history.get('turns', [])
    if not isinstance(turns, list):
        turns = []

    # Count existing user turns before appending the current message.
    # Finding #1 fix: the low-confidence create_session path seeds the first user turn
    # with the original raw_query (project.raw_query).  That seed must NOT count toward
    # the _CALIBRATION_TURN_CAP so that exactly _CALIBRATION_TURN_CAP genuine
    # calibrate() calls can occur before force-transition.
    prior_user_count = sum(1 for t in turns if t.get('role') == 'user')
    seed_offset = 1 if project.raw_query else 0

    # Append this message as a user turn
    turns.append({'role': 'user', 'text': message})

    # Re-run parse_query with accumulated conversation (user turn now appended)
    language = getattr(getattr(request.user, 'profile', None), 'language', None)
    parsed = services.parse_query(turns, language=language)

    # Extract calibration fields from parsed result
    new_confidence = parsed.get('confidence_score', 0.5)
    system_action = parsed.get('system_action', 'REQUEST_PRIORITY')
    ...
    # Update extracted_metadata with new parse result
    extracted_metadata = {
        k: filters.get(k)
        for k in ('program', 'material', 'style', 'atmosphere', 'location_country',
                  'color_tone', 'typology_primary')
    }
    extracted_metadata['priority_ordered'] = list(priority_ordered)

    # Append the LLM reply as a model turn for next calibration turn continuity.
    # Finding #3 fix: both appends happen before the single project.save() below.
    if llm_response_message:
        turns.append({'role': 'model', 'text': llm_response_message})

    # Single save for the full updated conversation (user turn + model reply).
    conv_history['turns'] = turns
    project.conversation_history = conv_history
    project.save(update_fields=['conversation_history'])

    # Decision: transition to exploring?
    # genuine_calibration_turns = total user turns after this message, minus the seed.
    genuine_calibration_turns = (prior_user_count + 1) - seed_offset
    force_transition = (genuine_calibration_turns >= _CALIBRATION_TURN_CAP)
    should_explore = force_transition or (new_confidence >= 0.60)

    if not should_explore:
        # Stay in chat_initializing — update session metadata and return next question
        session.confidence_score = new_confidence
        session.extracted_metadata = extracted_metadata
        session.priority_ordered = list(priority_ordered)
        session.save(update_fields=[
            'confidence_score', 'extracted_metadata', 'priority_ordered',
        ])
    ...
    # Build pool using the refined filters from the accumulated conversation.
    filter_priority_raw = parsed.get('filter_priority', [])
    filter_priority = [
        k for k in filter_priority_raw if isinstance(k, str) and k in _VALID_FILTER_KEYS
    ][:10]

    active_filters = dict(filters or {})
    req_focus = active_filters.get('image_focus')
    if req_focus not in _VALID_IMAGE_FOCUS:
        active_filters.pop('image_focus', None)

    # V_initial: try to use existing cached value; no HyDE call here (keep calibration fast)
    raw_query = parsed.get('raw_query') or ''
    v_initial = None
    if RC.get('stage_decouple_enabled', False):
        v_initial = services.get_cached_v_initial(request.user.id, raw_query or None)

    q_text_param = raw_query[:1000] if raw_query and RC.get('hybrid_retrieval_enabled', False) else None

    seed_ids = list(session.original_seed_ids or [])

    with stage('calibrate_create_pool', filter_keys=','.join(sorted(active_filters.keys()))):
        pool_ids, pool_scores, current_pool_tier = engine.create_pool_with_relaxation(
            active_filters, filter_priority, seed_ids,
            v_initial=v_initial, q_text=q_text_param,
        )
    ...
    if not pool_ids:
        # Finding #2 fix: do NOT strand the session with a terminal 404.
        # create_pool_with_relaxation already has a tier-3 random fallback, so an
        # empty result here means nothing publishable matched at all.  Keep the session
        # in chat_initializing so the user can broaden their criteria and call
        # calibrate() again (recoverable 200, not a loop of 404s).
        _broaden_message = (
            '선택하신 조건에 맞는 건물을 찾지 못했어요. '
            '조건을 조금 넓혀볼까요? (예: 지역, 용도, 재료 등)'
        )
        return Response({
            'session_id': str(session.session_id),
            'project_id': str(project.project_id),
            ...
            'extracted_metadata': extracted_metadata,
            'llm_response_message': _broaden_message,
            'suggested_quick_replies': [],
            'cards': [],
            'next_image': None,
            'prefetch_image': None,
    ...
    with stage('calibrate_get_embeddings', pool_size=len(pool_ids)):
        pool_embeddings = engine.get_pool_embeddings(pool_ids)
    ...
    image_focus = active_filters.get('image_focus')
    with stage('calibrate_fetch_cards', n=min(3, len(initial_batch))):
        _initial_cards = engine.get_buildings_by_ids(
            initial_batch[:3], image_focus=image_focus
        )
    first_card = _initial_cards[0] if len(_initial_cards) > 0 else None
    prefetch_card = _initial_cards[1] if len(_initial_cards) > 1 else None
    prefetch_card_2 = _initial_cards[2] if len(_initial_cards) > 2 else None

    # Persist session transition to exploring
    session.phase = 'exploring'
    session.pool_ids = pool_ids
    session.pool_scores = pool_scores
    session.initial_batch = initial_batch
    session.exposed_ids = [initial_batch[0]] if initial_batch else []
    session.current_pool_tier = current_pool_tier
    session.original_filters = active_filters
    session.original_filter_priority = list(filter_priority)
    session.original_q_text = q_text_param
    session.confidence_score = new_confidence
    session.extracted_metadata = extracted_metadata
    session.priority_ordered = list(priority_ordered)
    if v_initial is not None:
        session.v_initial = v_initial
    session.save(update_fields=[
        'phase', 'pool_ids', 'pool_scores', 'initial_batch', 'exposed_ids',
        'current_pool_tier', 'original_filters', 'original_filter_priority',
        'original_q_text', 'confidence_score', 'extracted_metadata', 'priority_ordered',
        'v_initial',
    ])

    evict_project_detail(str(project.project_id))

    # Seed prefetch cache for round 1
    _pf_seed = {
        'prefetch_card_id': initial_batch[1] if len(initial_batch) > 1 else None,
        'prefetch_card_2_id': initial_batch[2] if len(initial_batch) > 2 else None,
    }
    ...
    event_log.emit_event_batch([
        {
            ...
            'via_calibration': True,
            'calibration_turns': genuine_calibration_turns,
        },
    ])
